<?php
  ...
  if(isset($_GET["action"]) && $_GET["action"] == "delete") {
    ...
    $query = "DELETE FROM archive_invoices WHERE INVOICE_ID = $invoice_number";
  ...
  function showInvoices() {
    ...
    if($con) {
      $seq_no = 0;
      $query = "SELECT * FROM archive_invoices";
      $result = mysqli_query($con, $query);
  ...
  function showInvoiceRow($seq_no, $row) {
    ...
    <tr>
      ...
      <td>
        ...
        <button class="btn btn-primary btn-sm" onclick="restoreInvoice(<?php echo $row['INVOICE_ID']; ?>);">
