<?php
  ...
  function isAdmin() {
    ...
    if($con) {
      ...
      $row = mysqli_fetch_array($result);
      if($row)  {
        $query = "UPDATE user SET IS_LOGGED_IN = 'true'";
        $result = mysqli_query($con, $query);
        $_SESSION['id'] = $row['id'];
        echo "true";
  ...
  function storeAdminData() {
    ...
    if($con) {
      ...
      $query = "INSERT INTO user (PHARMACY_NAME, ADDRESS, EMAIL, CONTACT_NUMBER, USERNAME, PASSWORD, IS_LOGGED_IN) VALUES('$pharmacy_name', '$address', '$email', '$contact_number', '$username', '$password', 'false')";
      $result = mysqli_query($con, $query);
  ...
  function verifyEmailNumber() {
    ...
    if($con) {
      ...
      $query = "SELECT * FROM user WHERE EMAIL = '$email' AND CONTACT_NUMBER = '$contact_number'";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
      echo ($row) ? "true" : "false";
    }
  ...
  function updateUsernamePassword() {
    ...
    if($con) {
      ...
      $query = "UPDATE user SET USERNAME = '$username', PASSWORD = '$password' WHERE EMAIL = '$email' AND CONTACT_NUMBER = '$contact_number'";
      $result = mysqli_query($con, $query);
      echo ($result) ? "true" : "false";
    }
  }
  ...
  function updateAdminInfo() {
    ...
    if($con) {
      ...
      $query = "UPDATE user SET PHARMACY_NAME = '$pharmacy_name', ADDRESS = '$address', EMAIL = '$email', CONTACT_NUMBER = '$contact_number', USERNAME = '$username'";
      $result = mysqli_query($con, $query);
